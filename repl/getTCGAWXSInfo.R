...
setwd("~/Downloads/tcga/")

# read AR and VCF uuids from API portal query
portal = fread("TCGA_WXS_UUIDS.txt", h=T)


# read sar
sar = fread("tcga.sar.tsv", h=T)
names(sar)[1] = "sar.idx" 
sar$sar.idx = sar$sar.idx + 1

# read ar
ar = fread("tcga.ar.tsv", h=T)
names(ar)[1] = "ar.idx" 
ar$ar.idx = ar$ar.idx + 1
ar$ar_in_portal = ar$ar %in% portal$id

# read vcf
vcf = fread("tcga.vcf.tsv", h=T)
names(vcf)[1] = "vcf.idx" 
...
# [<AlignedReadsDataFromAlignmentCocleaningWorkflow((58d4d9ed-2aae-45b9-b7df-7732987fdc25)-[data_from]->(fb39f86f-e96c-4f4e-bcf2-925ee945f434)>,
#  <AlignedReadsDataFromAlignmentCocleaningWorkflow((66832985-f999-4020-8f55-50853e3d86dd)-[data_from]->(fb39f86f-e96c-4f4e-bcf2-925ee945f434)>]

# assume vcf.state == "submitted" old VarScan2 VCFs are removed
vcf = vcf[vcf.state == "released"]

# DAT-1489 vcf 205861a9-01c2-47a0-a3b3-52fb07c61bf3 is not in the portal (other non-portal VCF are all due to AR)
